...
/**
 * Fonction pour filtrer les lignes du tableau en fonction du type de paiement
 * @param {string} type - Type de paiement à afficher ('cheque', 'cb', 'especes', 'all')
 */
function filterTable(type) {
    const rows = document.querySelectorAll('#recettes tbody tr');
    rows.forEach(row => {
        if (type === 'all') {
            row.style.display = ''; // Affiche toutes les lignes
        } else if (row.classList.contains(type)) {
            row.style.display = ''; // Affiche les lignes correspondant au type
        } else {
            row.style.display = 'none'; // Masque les autres lignes
        }
    });
}
...
$(document).ready(function() {
    ...
    // Ajouter des écouteurs pour les boutons de filtrage
    $('body').on('click', '.filter-button', function() {
        const type = $(this).data('filter'); // Récupère le type de filtre depuis l'attribut data-filter
        filterTable(type);
    });
    ...
    $("#periodeQuickSelectID").on("change", function(e) {
        ...
        if (choix == 'today') {
            $('#beginPeriodeID').val(moment().format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().format('DD/MM/YYYY'));
        } else if (choix == 'yesterday') {
            $('#beginPeriodeID').val(moment().add(-1, 'days').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().add(-1, 'days').format('DD/MM/YYYY'));
        } else if (choix == 'thisweek') {
            $('#beginPeriodeID').val(moment().startOf('week').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().endOf('week').format('DD/MM/YYYY'));
        } else if (choix in {'thismonth':0, 'impayesmois':0} ) {
            $('#beginPeriodeID').val(moment().startOf('month').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().format('DD/MM/YYYY'));
        } else if (choix in {'lastmonth':0, 'bilanmois':0}) {
            $('#beginPeriodeID').val(moment().subtract(1, 'months').startOf('month').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().subtract(1, 'months').endOf('month').format('DD/MM/YYYY'));
        } else if (choix == 'lastweek') {
            $('#beginPeriodeID').val(moment().subtract(1, 'weeks').startOf('week').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().subtract(1, 'weeks').endOf('week').format('DD/MM/YYYY'));
        } else if (choix in {'thisyear':0, 'impayesannee':0}) {
            $('#beginPeriodeID').val(moment().startOf('year').format('DD/MM/YYYY'));
            $('#endPeriodeID').val(moment().format('DD/MM/YYYY'));
        } else if (choix in {'lastyear':0, 'bilanannee':0}) {
            $('#beginPeriodeID').val(moment().subtract(1, 'years').format('01/01/YYYY'));
            $('#endPeriodeID').val(moment().subtract(1, 'years').format('31/12/YYYY'));
        }
