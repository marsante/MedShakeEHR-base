...
function getFseData(el) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      <th class="text-start" colspan="2">Actes - modificateurs</th> \
      <th ></th> \
      <th >Code asso.</th> \
      <th>P.U.</th> \
      <th>%</th> \
      ...
      $.each(data.actes, function(k, v) {
        ...
        html += '<tr> \
        <td class="text-start">' + (v.qte > 1 ? v.qte : '') + v.acte + '</td> \
        <td>' + v.codeQualif + '</td> \
        <td>' + v.modifsCCAM + '</td> \
        <td>' + v.codeAsso + '</td> \
        <td>' + v.base + ' €</td> \
...
function fseWait() {
  ...
  waitingMsg = '<div class="text-center"><div class="spinner-grow text-success" style="width: 3rem; height: 3rem;" role="status"><span class="visually-hidden">Attente du retour</span></div><div>Nous attendons le retour d\'informations concernant la FSE établie.<br>Ce message sera mis à jour automatiquement.</div></div>';
...
function fseEnd(data) {
  if (data.aPayerError || data.totalError) {
    ...
    <span class="d-block fw-bold mt-2">Il existe un différentiel entre le règlement enregistré et la FSE réalisée.<br>Éditez votre règlement pour corriger cette situation.</span> \
    <table class="table table-sm table-hover my-3"> \
      <thead class="table-light"><tr><th></th><th>Règlement</th><th>FSE</th></tr></thead> \
      <tbody> \
      <tr><td class="fw-bold">Actes</td><td>' + data.actesEHR + '</td><td>' + data.actesFSE + '</td></tr> \
      <tr><td class="fw-bold">Montant total</td><td>' + data.totalEHR + '</td><td>' + data.totalFSE + '</td></tr> \
      <tr><td class="fw-bold">À régler</td><td>' + data.aPayerEHR + '</td><td>' + data.aPayerFSE + '</td></tr> \
      </tbody></table> \
    </div>';
  } else {
    endMsg = '<div class="text-center"><i class="fas fa-check-circle text-success fa-4x"></i><span class="d-block fw-bold mt-2">Le processus s\'est terminé correctement !</span></div>';
  }
  ...
  $(dataVitale).each(function(index, dat) {
    ...
    if (mode == "classique") {
      if (dat.correspondances && dat.correspondances.length == 1) {
        html += '<a href="/patient/' + dat['correspondances'][0] + '/" class="btn btn-sm btn-danger ms-1 goToPatientFromVitaleData" title="Ouvrir le dossier"><i class="fas fa-folder-open"></i></a>';
      }
      ...
      if (!dat.correspondances) {
        ...
        html += '<button type="submit" class="btn btn-sm btn-warning ms-1" title="Créer le dossier"><i class="fas fa-user-plus"></i></button>';
        html += '</form>';
      }

      html += '<button type="button" data-indexVitale="' + index + '" class="btn btn-sm btn-secondary ms-1 searchPatientFromVitaleDataNss" title="Rechercher le dossier avec nom et prénom"><i class="fas fa-search"></i></button>';

    } else {
      if (dat.correspondances && dat.correspondances.length == 1) {
        html += '<a href="/patient/' + dat['correspondances'][0] + '/" class="btn btn-sm btn-danger ms-1 goToPatientFromVitaleData" title="Ouvrir le dossier"><i class="fas fa-folder-open"></i></a>';
      } else {
        html += '<button type="button" class="btn btn-sm btn-warning ms-1" title="Créer le dossier"><i class="fas fa-user-plus"></i></button>';
      }
    }
    html += '</div>\
    </div></div>';
