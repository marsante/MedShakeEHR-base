...
//sortir les reglements

$porteurRegIDImplode = msSQL::sqlGetTagsForWhereIn($porteursReglementIds, 'porteurRegID');
$pratsSelectIDImplode = msSQL::sqlGetTagsForWhereIn($p['page']['pratsSelect'], 'pratSelectID');
$marqueurs = array_merge($porteurRegIDImplode['execute'], $pratsSelectIDImplode['execute'], $name2typeID);

$marqueurs['beginPeriode'] = $beginPeriode->format("Y-m-d");
$marqueurs['endPeriode'] = $endPeriode->format("Y-m-d");


$sql = "SELECT pd.toID, pd.id, pd.typeID, pd.value, pd.creationDate, pd.registerDate, pd.instance, p.value as prenom , a.label, dc.name,
...
pd.id in (
  ...
  where pd1.typeID in (" . $porteurRegIDImplode['in'] . ") and DATE(pd1.creationDate) >= :beginPeriode and DATE(pd1.creationDate) <= :endPeriode and pd1.deleted='' and pd1.fromID in (" . $pratsSelectIDImplode['in'] . ")"
...
pd.instance in (
  ...
  where pd2.typeID in (" . $porteurRegIDImplode['in'] . ") and DATE(pd2.creationDate) >= :beginPeriode and DATE(pd2.creationDate) <= :endPeriode and pd2.deleted='' and pd2.fromID in (" . $pratsSelectIDImplode['in'] . ")"
	. ($impayes ? "and important='y'" : "") . "
)
